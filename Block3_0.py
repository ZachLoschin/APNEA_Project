...
# data_dir = pathlib.Path("Z:/ResearchHome/ClusterHome/asanch24/APNEA/APNEA Raw Files/Test_EDF/")
...
with open(file, 'rb') as f:
    ...
    plt.clf()
    # plt.plot(data["EEG_ArtZero"][start:finish])
    # plt.ylim(-200, 200)
    # plt.show()
    ...
    # plt.clf()
    # plt.plot(data["EEG_Filt"][start:finish])
    ...
    # # Define path
    # dict_file = 'Spindles_ID16_250_NREM.xlsx'
    ...
    print("Saving to excel...")
    summary.to_excel('Pre_Removal.xlsx', index=False)
    ...
    print("Saving to excel...")
    summary.to_excel('Post_Removal.xlsx', index=False)
    ...
    Axis[1].set_yticks(ticks=[0, 1, 2, 3, 4], labels=["N3", "N2", "N1", "R", "W"])
    ...
    if len(sum30) == len(hypno_enum):
        print("***SUM AND HYPNOENUM ARE EQUAL***")
...
SpindlePerc.to_excel('SpindlePerc.xlsx')
